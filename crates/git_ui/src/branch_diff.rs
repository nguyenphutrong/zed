use crate::{
    ...
    project_diff::{self, DeployBranchDiff, ReviewDiff, render_send_review_to_agent_button},
};
...
use project::{
    ...
    git_store::{
        Repository,
        diff_buffer_list::{self, DiffBase, DiffScope},
    },
};
...
impl BranchDiff {
    ...
    fn new_with_scope(
        ...
        let diff_buffer_list = cx.new(|cx| {
            diff_buffer_list::DiffBufferList::new_with_scope(
                diff_base,
                scope,
                repo,
                project.clone(),
        ...
        let diff_buffer_list_for_addon = diff_buffer_list.clone();
        let diff = cx.new(|cx| {
            DiffMultibuffer::new(
                diff_buffer_list,
                Capability::ReadOnly,
                ...
                move |editor, cx| {
                    editor.rhs_editor().update(cx, move |rhs_editor, _cx| {
                        ...
                        rhs_editor.register_addon(BranchDiffAddon {
                            branch_diff: diff_buffer_list_for_addon,
                        });
                    });
                },
                project.clone(),
...
impl Item for BranchDiff {
    ...
    fn tab_content_text(&self, _detail: usize, cx: &App) -> SharedString {
        match self.diff_base(cx) {
            ...
            DiffBase::Compare { base_label, .. } => format!("Compare with {}", base_label).into(),
    ...
    async fn test_branch_diff_action_matches_existing_item_by_base_ref(cx: &mut TestAppContext) {
        ...
        let (active_base_ref, mut base_refs) = workspace.update(cx, |workspace, cx| {
            ...
            let active_base_ref = match active_item.read(cx).diff_base(cx) {
                ...
                DiffBase::Compare { .. } => panic!("expected active item to be a branch diff"),
            };
