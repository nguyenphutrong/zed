...
use git::{
    ...
    repository::{
        ...
        LogSource, PushOptions, Remote, RepoPath, ResetMode, SearchCommitArgs, UpstreamTracking,
    },
...
use ui::{
    ...
    IndentGuideColors, ListItem, ListItemSpacing, PopoverMenu, PopoverMenuHandle,
...
actions!(
    ...
    [
        ...
        FocusSearch,
        /// Toggles the branch filter picker.
        ToggleBranchFilter,
...
impl PushBranchDialogState {
    fn new(branch: Branch, available_remotes: Vec<SharedString>) -> anyhow::Result<Self> {
        ...
        let push_mode =
            if Self::tracked_upstream_remote_name(&branch) == Some(selected_remote.as_ref()) {
                BranchPushMode::Normal
            } else {
                BranchPushMode::SetUpstream
            };
    ...
    fn select_remote(&mut self, remote_name: SharedString) {
        ...
        self.push_mode = if Self::tracked_upstream_remote_name(&self.branch)
    ...
    fn push_target(&self) -> BranchPushTarget {
        ...
        BranchPushTarget {
            branch_name: self.branch.name().to_string().into(),
            remote_branch_name,
            remote: Remote {
                name: self.selected_remote.clone(),
            },
            option: self.push_mode.option(),
        }
    }
}
...
pub struct GitGraph {
    ...
    branch_filter_picker: Option<Entity<Picker<BranchFilterPickerDelegate>>>,
...
impl GitGraph {
    ...
    fn is_visible_ref_name(&self, ref_name: &str) -> bool {
        ...
        if !self.settings_dropdown_state.settings.show_remote_branches
            && !Self::ref_name_matches_remote_visibility(
                false,
    ...
    fn all_branches_selection_state(&self) -> ToggleState {
        Self::branch_selection_state_for_state(
    ...
    fn branch_selection_state_for_state(
        branches_loaded: bool,
        show_remote_branches: bool,
    ...
    fn all_branches_selected(&self) -> bool {
        self.all_branches_selection_state() == ToggleState::Selected
    }
    ...
    fn load_available_branches(&mut self, window: &mut Window, cx: &mut Context<Self>) {
    ...
    fn refresh_branch_filter_picker(&mut self, cx: &mut Context<Self>) {
        ...
        if let Some(picker) = &self.branch_filter_picker {
    ...
    fn open_branch_filter(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        let picker = self.ensure_branch_filter_picker(window, cx);
        if !self.branch_filter_state.branches_loaded {
            self.load_available_branches(window, cx);
        }
    ...
    fn ref_name_matches_remote_visibility(
        ...
        if Self::is_remote_branch_ref_name(ref_name) {
            return false;
        }
        ...
        !available_branches
            ...
            .map(|branch| Self::branch_display_name(branch.ref_name.as_ref()))
            .any(|display_name| {
                ref_name == display_name.as_ref()
                    || ref_name
                        .strip_prefix("HEAD -> ")
                        .is_some_and(|name| name == display_name.as_ref())
            })
    }
    ...
    fn update_graph_settings(
        ...
        let old_show_remote_branches = settings.show_remote_branches;
        let previously_all_eligible_selected = self.all_branches_selected();
        let previous_selection = self.branch_filter_state.selected_branches.clone();
        update(&mut settings);
        ...
        if old_show_remote_branches != settings.show_remote_branches
            && self.branch_filter_state.branches_loaded
        {
    ...
    fn ensure_branch_filter_picker(
        &mut self,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) -> Entity<Picker<BranchFilterPickerDelegate>> {
        if let Some(picker) = &self.branch_filter_picker {
            return picker.clone();
        }

        let weak_graph = cx.weak_entity();
        let query = self.branch_filter_state.query.to_string();
        let matches = self.branch_filter_entries(&query);
        let picker = cx.new(|cx| {
            let mut delegate = BranchFilterPickerDelegate::new();
            delegate.graph = Some(weak_graph);
                    ...
                    div()
                        ...
                        .when(!has_context_menu, |this| {
                            if let CommitDataState::Loaded(commit_data) = &data {
                                ...
                                let tag_names: Vec<SharedString> = commit
                                    .data
                                    .ref_names
                                    .iter()
                                    .filter_map(|name| name.strip_prefix("tag: "))
    ...
    fn push_branch(&mut self, branch: Branch, window: &mut Window, cx: &mut Context<Self>) {
        if branch.is_remote() {
            return;
        }
        let Some(repository) = self.get_repository(cx) else {
    ...
    fn perform_push_branch(
        ...
        self.run_git_operation(operation, "Failed to push branch", window, cx);
    }

    fn push_tag(&mut self, tag_name: SharedString, window: &mut Window, cx: &mut Context<Self>) {
        let Some(repository) = self.get_repository(cx) else {
            return;
        };
    ...
    fn perform_push_tag(
        ...
        let operation = cx.spawn(async move |_, cx| {
            repository
                .update(cx, |repository, cx| {
                    repository.push(tag_name.clone(), tag_name, remote_name, None, askpass, cx)
                })
                .await
                .map_err(|_| anyhow::anyhow!("Operation was canceled"))??;
            anyhow::Ok(())
        ...
        self.run_git_operation(operation, "Failed to push tag", window, cx);
    }
        ...
        let context_menu = ContextMenu::build(window, cx, move |context_menu, window, _| {
            let menu = match ref_kind.clone() {
                RefNameKind::Branch {
                    ...
                    let menu =
                        if let Some(branch) = push_branch.filter(|branch| !branch.is_remote()) {
                            menu.entry("Push Branch...", None, {
                                let git_graph = git_graph.clone();
                                move |window, cx| {
                                    git_graph.update(cx, |this, cx| {
                                        this.push_branch(branch.clone(), window, cx);
                                    });
                                }
                            })
                        } else {
                ...
                RefNameKind::Tag(tag_name) => {
                    ...
                    let push_tag_name = tag_name.clone();
                    ...
                    context_menu
                        ...
                        .entry("Push Tag...", None, {
                            ...
                            move |window, cx| {
                                git_graph.update(cx, |this, cx| {
                                    this.push_tag(push_tag_name.clone(), window, cx);
                                });
                            }
                        })
    ...
    fn set_context_menu(
        ...
        self.context_menu = Some(GitGraphContextMenu {
            menu: context_menu,
            position,
            target_entry_index: Some(entry_idx),
            _subscription: subscription,
        });
        cx.notify();
    }
    ...
    fn handle_context_menu_overlay_secondary_mouse_down(
        ...
        if let Some(row) = self.row_at_position(event.position.y, window, cx) {
            self.deploy_entry_context_menu(event.position, row, None, window, cx);
        } else {
        ...
        v_flex()
            ...
            .child(
                h_flex()
                    ...
                    .child(Icon::new(IconName::GitBranch).size(IconSize::XSmall))
                    .child(
                        Headline::new(format!("Push Branch ({})", self.state.branch.name()))
                            .size(HeadlineSize::XSmall),
                    ),
            )
            .child(
                v_flex()
                    ...
                    .child(
                        v_flex()
                            .gap_1()
                            .child(Label::new("Remote").size(LabelSize::Small))
                            .child(self.render_remote_dropdown(window, cx)),
                    )
                    .child(
                        v_flex()
                            .gap_1()
                            .child(Label::new("Mode").size(LabelSize::Small))
        ...
        v_flex()
            ...
            .child(
                h_flex()
                    ...
                    .child(
                        Headline::new(format!("Push Tag ({})", self.state.tag_name))
                            .size(HeadlineSize::XSmall),
                    ),
            )
            .child(
...
impl PickerDelegate for BranchFilterPickerDelegate {
    ...
    fn update_matches(
        ...
        if let Some(graph) = self.graph.as_ref().and_then(|graph| graph.upgrade()) {
            ...
            let query_for_graph = SharedString::from(query.clone());
        ...
        if let Some(graph) = self.graph.as_ref().and_then(|graph| graph.upgrade()) {
            ...
            window.defer(cx, move |_window, cx| {
                if let Some(graph) = graph.upgrade() {
                    graph.update(cx, |graph, cx| {
                        graph.branch_filter_state.handle.hide(cx);
                    });
                }
            });
        }
...
mod persistence {
    ...
    pub fn serialize_log_source_type(log_source: &LogSource) -> i32 {
        match log_source.base_source() {
            LogSource::All | LogSource::Branches(_) => LOG_SOURCE_ALL,
    ...
    pub fn serialize_log_source_value(log_source: &LogSource) -> Option<String> {
        match log_source.base_source() {
            ...
            LogSource::Branches(branches) => Some(branches.join("\n")),
...
mod tests {
    ...
    fn branch_infos(ref_names: &[&str]) -> Vec<BranchInfo> {
        ref_names
            .iter()
            .map(|ref_name| BranchInfo {
                ref_name: SharedString::from(*ref_name),
                is_head: *ref_name == "refs/heads/main",
                is_remote: ref_name.starts_with("refs/remotes/"),
                is_selected: false,
            })
            .collect()
    }

    fn branch_ref_set(ref_names: &[&str]) -> HashSet<SharedString> {
        ref_names
            .iter()
            .map(|ref_name| SharedString::from(*ref_name))
            .collect()
    }

    fn branch_ref_vec(ref_names: &[&str]) -> Vec<SharedString> {
        ref_names
            .iter()
            .map(|ref_name| SharedString::from(*ref_name))
            .collect()
    ...
    async fn setup_git_graph_with_branches(
    ...
    ) -> (Entity<GitGraph>, gpui::AnyWindowHandle) {
        ...
        let workspace_weak = multi_workspace.read_with(&*cx, |multi_workspace, _| {
    ...
    async fn test_graph_scroll_is_blocked_while_context_menu_is_open(cx: &mut TestAppContext) {
        ...
        git_graph.update_in(cx, |graph, window, cx| {
            ...
            graph.deploy_entry_context_menu(point(px(10.), px(10.)), 0, None, window, cx);
    ...
    async fn test_right_clicking_another_commit_replaces_open_context_menu(
        ...
        git_graph.update_in(cx, |graph, window, cx| {
            ...
            graph.deploy_entry_context_menu(point(px(10.), row_height * 0.5), 0, None, window, cx);
            assert_eq!(
                graph
                    .context_menu
                    .as_ref()
    ...
    #[test]
    fn test_branch_filter_source_uses_all_when_every_visible_ref_is_selected() {
        let available_branches = branch_infos(&[
            "refs/heads/main",
            "refs/heads/feature",
            "refs/remotes/origin/main",
    ...
    #[test]
    fn test_branch_filter_selection_state_tracks_all_none_and_indeterminate() {
        let available_branches = branch_infos(&[
            "refs/heads/main",
            "refs/heads/feature",
            "refs/remotes/origin/main",
        ...
        assert_eq!(
            GitGraph::branch_selection_state_for_state(
                true,
                true,
                &available_branches,
                &branch_ref_set(&[
        ...
        assert_eq!(
            GitGraph::branch_selection_state_for_state(
                ...
                true,
                &available_branches,
                &branch_ref_set(&["refs/heads/main"]),
            ),
            ToggleState::Indeterminate
        );
    }
    ...
    #[test]
    fn test_ref_name_matches_remote_visibility_hides_remote_display_refs() {
        let available_branches = branch_infos(&["refs/heads/main", "refs/remotes/origin/main"]);
    ...
    #[test]
    fn test_branch_filter_reconciles_selection_after_branch_reload() {
        let previous_selection = branch_ref_set(&[
            "refs/heads/main",
            "refs/heads/deleted",
            "refs/remotes/origin/main",
        ]);
        let eligible_ref_names = branch_ref_set(&["refs/heads/main", "refs/remotes/origin/main"]);
        ...
        assert_eq!(
            reconciled,
            branch_ref_set(&["refs/heads/main", "refs/remotes/origin/main"])
        );
    }

    #[gpui::test]
    async fn test_branch_filter_entries_exposes_more_than_two_hundred_branches(
        cx: &mut TestAppContext,
    ) {
        init_test(cx);

        let (git_graph, _window_handle) = setup_git_graph_with_branches(&["main"], cx).await;
    ...
    fn push_branch_dialog_defaults_to_tracked_remote() {
        ...
        assert_eq!(state.push_mode, BranchPushMode::Normal);

        let target = state.push_target();
        assert_eq!(target.branch_name.as_ref(), "feature");
    ...
    #[test]
    fn push_branch_dialog_selects_another_remote_and_sets_upstream() {
    ...
    #[test]
    fn push_branch_dialog_supports_force_with_lease() {
        let mut state = PushBranchDialogState::new(
        ...
        state.push_mode = BranchPushMode::ForceWithLease;
    ...
    #[test]
    fn push_tag_dialog_requires_remote() {
