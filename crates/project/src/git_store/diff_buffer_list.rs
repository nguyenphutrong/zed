        ...
        self.project.update(cx, |_project, cx| {
            ...
            } = &self.scope
            {
                let tree_status = self
                    ...
                    .cloned();
                let Some(tree_status) = tree_status else {
                    return;
                };
                let task = Self::load_buffer(
                    self.diff_base.clone(),
                    Some(tree_status.clone()),
                    project_path.clone(),
                    repo.clone(),
                    cx,
                );
                output.push(DiffBuffer {
                    ...
                    load: task,
                    file_status: diff_status_to_file_status(&tree_status),
                });
                return;
            }
            ...
            for item in repo.read(cx).cached_status() {
                ...
                let Some(status) = (match self.diff_base {
                    DiffBase::Head | DiffBase::Merge { .. } | DiffBase::Compare { .. } => {
